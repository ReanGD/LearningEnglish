...
class GlobalStatistic:
	...
	def add_word(self, word, stat_en_ru, stat_ru_en):
		self.stat_en_ru.append((self.calc_stat(word, stat_en_ru), word))
		self.stat_ru_en.append((self.calc_stat(word, stat_ru_en), word))
	...
	def get_common_stat(self):
		...
		for i in range(0, 3):
			cnt_ru_en  = sum(1 for row, word in self.stat_ru_en if row[6] == unicode(i))
			pers_ru_en = round(float(cnt_ru_en) * 100.0 / float(total_ru_en), 2)
			cnt_en_ru  = sum(1 for row, word in self.stat_en_ru if row[6] == unicode(i))
