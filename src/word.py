...
class Word:
	...
	def __init__(self):
		...
		self.en_source     = ""  # Исходное английское слово, без преобразований
		self.en_word_list  = []  # Распарсенное английское слово
		self.transcription = ""  # Транскрипция
		self.ru_source     = []  # Массив русских слов, без преобразований
	...
	def add_value(self, en_word, transcription, ru_word):
		...
		self.ru_word_list = map(lambda x: Word._convert_spec_chars(reg_no_sign_part.sub(".*?", reg_cmnt.sub("", x.lower()).strip())), self.ru_source)
